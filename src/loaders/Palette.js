/**
 * Quake color palette - 256 indexed colors
 * This is the default Quake palette (palette.lmp)
 */
...
/**
 * Convert indexed color pixel data to RGBA
 * No vertical flip - Quake's UV system (T=0 at top) combined with
 * OpenGL's texture storage (row 0 at V=0) means UVs map correctly
 */
export function indexedToRGBA(indexedData, width, height, palette = QUAKE_PALETTE) {
    ...
    for (let i = 0; i < indexedData.length; i++) {
        ...
        // Index 255 is transparent in Quake
        rgba[rgbaOffset + 3] = colorIndex === 255 ? 0 : 255;
    }
...
/**
 * Create a DataTexture from indexed pixel data
 */
export function createTextureFromIndexed(indexedData, width, height, THREE, palette = QUAKE_PALETTE) {
    const rgba = indexedToRGBA(indexedData, width, height, palette);
